...
def fetch_activation_bytes(username, password, options):
    ...
    if lang == "uk":  # something more clever might be needed
	login_url = login_url.replace(".com", ".co.uk")
        base_url = base_url.replace(".com", ".co.uk")
    else:  # something more clever might be needed
    ...
    driver.get(url)
    if os.getenv("DEBUG") or options.debug or True:  # enable if you hit CAPTCHA or 2FA or other "security" screens
